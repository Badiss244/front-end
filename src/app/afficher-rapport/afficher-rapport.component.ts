...
@Component({
  selector: 'app-afficher-rapport',
  imports: [CommonModule],
  templateUrl: './afficher-rapport.component.html'
})
export class AfficherRapportComponent implements OnInit {
  reports: Report[] = [];
  errorMessage: string = '';
  ...
  fetchReports(): void {
    ...
    this.http.get<Report[]>('https://localhost:7299/api/Auditor/rapports', { headers })
      .subscribe({
        next: (data) => {
          this.reports = data;
          this.loading = false;
